...
function Spartan_BR_Actions_True_DetailInsertRow(rowIndex) {
    if (EjecutarValidacionesAntesDeGuardarMRSpartan_BR_Actions_True_Detail("Spartan_BR_Actions_True_Detail_", "_" + rowIndex)) {
        ...
        var counter = 1;
        var parameter1Value = GetParameterValue(4, data);
        var parameter2Value = GetParameterValue(5, data);
        var parameter3Value = GetParameterValue(6, data);
        var parameter4Value = GetParameterValue(7, data);
        var parameter5Value = GetParameterValue(8, data);
        //MULTISELECTNEW
        var newData = {
            ...
            , Parameter_1: parameter1Value
            , Parameter_2: parameter2Value
            , Parameter_3: parameter3Value
            , Parameter_4: parameter4Value
            , Parameter_5: parameter5Value
...
//MULTISELECTNEW
function GetParameterValue(id, data) {
    var parameterValue;
    if ($(data.childNodes[id].childNodes[1]).hasClass('multiselect')) {
        var r = '';
        $.each($(data.childNodes[id].childNodes[1]).select2('data'), function (i, v) {
            r += v.text + ';';
        });
        if (r !== '')
        ...
        if (data.childNodes[id].childNodes.length == 2) {
            parameterValue = '';
        }
        else {
            if (data.childNodes[id].childNodes[1].value == '') {
                parameterValue = data.childNodes[id].childNodes[2].value;
            }
            else {
                parameterValue = $('#' + $(data.childNodes[id].childNodes[1])[0].id + ' option:selected').text();
            }
        }
    }
...
function Spartan_BR_Actions_True_DetailEditRow(rowIndex, currentRow, executeRules) {
   ...
   //MULTISELECTNEW
    var strings = [];
    var values = [];
    if ($('#ddlParameter_1_' + rowIndex).hasClass('multiselect')) {
        strings = prevData.Parameter_1.split(';');
        values = [];
        $.each(strings, function (i, s) {
            values.push($($('#ddlParameter_1_' + rowIndex + ' option:contains(' + s + ')')[0]).val());
        });
        $('#ddlParameter_1_' + rowIndex).val(values).change();
    }
    else {
    ...
    if ($('#ddlParameter_2_' + rowIndex).hasClass('multiselect')) {
        strings = prevData.Parameter_2.split(';');
        values = [];
        $.each(strings, function (i, s) {
            values.push($($('#ddlParameter_2_' + rowIndex + ' option:contains(' + s + ')')[0]).val());
        });
        $('#ddlParameter_2_' + rowIndex).val(values).change();
    }
    else {
