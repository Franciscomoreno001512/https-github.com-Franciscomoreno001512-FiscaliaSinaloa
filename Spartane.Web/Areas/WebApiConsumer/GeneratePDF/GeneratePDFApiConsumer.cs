...
namespace Spartane.Web.Areas.WebApiConsumer.GeneratePDF
{
    public class GeneratePDFApiConsumer : BaseApiConsumer, IGeneratePDFApiConsumer
    {
        ...
        //fjmore
        public ApiResponse<string[]> GetHtmlAsString(int idFormat, string RecordId)
        {
            try
            {
                var varRecords = RestApiHelper.InvokeApi<string[]>(baseApi, ApiControllerUrl + "/GetHtmlAsString?idFormat=" + idFormat + "&RecordId=" + RecordId + "&ImgDirectory=" + DirectoryImg, Method.GET, ApiHeader);
        ...
        public ApiResponse<string> GeneratePDFFromControlDocumentos(int FolioControlDocumentos)
        {
            try
            {
                var varRecords = RestApiHelper.InvokeApi<string>(baseApi, ApiControllerUrl + "/GeneratePDFFromControlDocumentos?FolioControlDocumentos=" + FolioControlDocumentos, Method.GET, ApiHeader);
                return new ApiResponse<string>(true, varRecords);
            }
            catch (Exception)
            {
