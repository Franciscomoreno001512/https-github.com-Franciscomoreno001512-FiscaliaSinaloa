...
namespace Spartane.Web.Areas.Frontal.Controllers
{
    ...
    public class Spartan_FormatController : Controller
    {
        ...
        //fjmore
        [HttpGet]
        public ActionResult GetHtmlAsString(int idFormat, string RecordId)
        {
            if (!_tokenManager.GenerateToken())
                return null;

            _IGeneratePDFApiConsumer.SetAuthHeader(_tokenManager.Token);
            var dataHtml = _IGeneratePDFApiConsumer.GetHtmlAsString(idFormat, RecordId).Resource;
            ...
            if (dataHtml != null)
            {
                string he = dataHtml[0];
                string body = dataHtml[1];
                string foo = dataHtml[2];
                var rh = _ISpartaneQueryApiConsumer.ExecuteRawQuery(string.Format("exec uso_InsertHtmlElementsTemp '{0}','{1}', '{2}','{3}'", guid, he, body, foo)).Resource;
        ...
        [HttpGet]
        public ActionResult GeneratePDFFromControlDocumentos(int FolioControlDocumentos)
        {
            if (!_tokenManager.GenerateToken())
                return null;

            _IGeneratePDFApiConsumer.SetAuthHeader(_tokenManager.Token);
            var res = _IGeneratePDFApiConsumer.GeneratePDFFromControlDocumentos(FolioControlDocumentos).Resource;
