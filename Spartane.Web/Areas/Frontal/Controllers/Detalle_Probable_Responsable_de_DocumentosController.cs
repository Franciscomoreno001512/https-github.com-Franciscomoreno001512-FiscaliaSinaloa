...
namespace Spartane.Web.Areas.Frontal.Controllers
{
    ...
    public class Detalle_Probable_Responsable_de_DocumentosController : Controller
    {
        ...
        public ActionResult Index(int ModuleId=0)
        {
            ...
            var permission = PermissionHelper.GetRoleObjectPermission(SessionHelper.Role, 45389, ModuleId);
        ...
        public ActionResult Create(int Id = 0,  int consult = 0, int ModuleId=0)
        {
           ...
           if ((!permission.New && Id.ToString() == "0") || (!permission.Edit && Id.ToString() != "0" && (!permission.Consult && consult == 1)))
			...
			varDetalle_Probable_Responsable_de_Documentos.Clave = Id;
            ...
            if ((Id.GetType() == typeof(string) && Id.ToString() != "") || ((Id.GetType() == typeof(int) || Id.GetType() == typeof(Int16) || Id.GetType() == typeof(Int32) || Id.GetType() == typeof(Int64) || Id.GetType() == typeof(short))&& Id.ToString() != "0"))
            {
				...
				if (Detalle_Probable_Responsable_de_DocumentossData != null && Detalle_Probable_Responsable_de_DocumentossData.Count > 0)
                {
					...
					varDetalle_Probable_Responsable_de_Documentos= new Detalle_Probable_Responsable_de_DocumentosModel
					{
	                    ...
	                    ,IdDocumentos = Detalle_Probable_Responsable_de_DocumentosData.IdDocumentos
	...
	[HttpGet]
        public ActionResult AddDetalle_Probable_Responsable_de_Documentos(int rowIndex = 0, int functionMode = 0, string id = "0")
        {
        ...
        [HttpPost]
        public ActionResult ShowAdvanceFilter(Detalle_Probable_Responsable_de_DocumentosAdvanceSearchModel model, int idFilter = -1)
        {
        ...
        public ActionResult ShowAdvanceFilter(string previousFilters = "")
        {
            ...
            var previousFiltersObj = new Detalle_Probable_Responsable_de_DocumentosAdvanceSearchModel();
            if (previousFilters != "")
            {
                previousFiltersObj = (Detalle_Probable_Responsable_de_DocumentosAdvanceSearchModel)(Session["AdvanceSearch"] ?? new Detalle_Probable_Responsable_de_DocumentosAdvanceSearchModel());
            }
		...
		/*
		 [HttpGet]
        public ActionResult GetDetalle_Probable_Responsable_de_DocumentosAll()
        {
            try
            {
                if (!_tokenManager.GenerateToken())
                    return Json(null, JsonRequestBehavior.AllowGet);
                _IDetalle_Probable_Responsable_de_DocumentosApiConsumer.SetAuthHeader(_tokenManager.Token);
                var result = _IDetalle_Probable_Responsable_de_DocumentosApiConsumer.SelAll(false).Resource;
                
                return Json(result.OrderBy(m => m.).Select(m => new SelectListItem
                {
                     Text = CultureHelper.GetTraductionNew(Convert.ToString(m.Clave), "Detalle_Probable_Responsable_de_Documentos", m.),
                     Value = Convert.ToString(m.Clave)
                }).ToArray(), JsonRequestBehavior.AllowGet);
            }
            catch (ServiceException ex)
...
*/
        /// <summary>
        /// Get List of Detalle_Probable_Responsable_de_Documentos from Web API.
        /// </summary>
        /// <param name="draw"></param>
        /// <param name="start"></param>
        /// <param name="length"></param>
        /// <returns>Return List of Detalle_Probable_Responsable_de_Documentos Entity</returns>
        public ActionResult GetDetalle_Probable_Responsable_de_DocumentosList(UrlParametersModel param)
        {
            ...
            var configuration = new GridConfiguration() { OrderByClause = "", WhereClause = "" };
            if (filter != null)
                configuration = GridQueryHelper.GetDataTableConfigurationNew(param, new Detalle_Probable_Responsable_de_DocumentosPropertyMapper());
            ...
            {
				if (Session["AdvanceSearch"].GetType() == typeof(Detalle_Probable_Responsable_de_DocumentosAdvanceSearchModel))
                {
					var advanceFilter =
                    (Detalle_Probable_Responsable_de_DocumentosAdvanceSearchModel)Session["AdvanceSearch"];
            ...
            Detalle_Probable_Responsable_de_DocumentosPropertyMapper oDetalle_Probable_Responsable_de_DocumentosPropertyMapper = new Detalle_Probable_Responsable_de_DocumentosPropertyMapper();
			if (String.IsNullOrEmpty(order))
            {
                 if (sortColumn != -1)
                    configuration.OrderByClause = oDetalle_Probable_Responsable_de_DocumentosPropertyMapper.GetPropertyName(param.columns[sortColumn].name) + " " + sortDirection;
            }

            var pageSize = iDisplayLength;
            var pageIndex = (iDisplayStart / iDisplayLength) + 1;
            var result = _IDetalle_Probable_Responsable_de_DocumentosApiConsumer.ListaSelAll((pageIndex * pageSize) - pageSize + 1, pageSize + ((pageIndex * pageSize) - pageSize), configuration.WhereClause, configuration.OrderByClause ?? "").Resource;
            if (result.Detalle_Probable_Responsable_de_Documentoss == null)
                result.Detalle_Probable_Responsable_de_Documentoss = new List<Detalle_Probable_Responsable_de_Documentos>();

            return Json(new
            {
                aaData = result.Detalle_Probable_Responsable_de_Documentoss.Select(m => new Detalle_Probable_Responsable_de_DocumentosGridModel
            {
                    Clave = m.Clave
			,IdDocumentos = m.IdDocumentos
                        ,Probable_ResponsableNombre_Completo_del_Tutor = CultureHelper.GetTraduction(m.Probable_Responsable_Detalle_de_Imputado.Clave.ToString(), "Detalle_de_Imputado") ?? (string)m.Probable_Responsable_Detalle_de_Imputado.Nombre_Completo_del_Tutor
        ...
        [NonAction]
        public string GetAdvanceFilter(Detalle_Probable_Responsable_de_DocumentosAdvanceSearchModel filter)
        {
            var where = "";
            if (!string.IsNullOrEmpty(filter.FromClave) || !string.IsNullOrEmpty(filter.ToClave))
            {
                if (!string.IsNullOrEmpty(filter.FromClave))
                    where += " AND Detalle_Probable_Responsable_de_Documentos.Clave >= " + filter.FromClave;
                if (!string.IsNullOrEmpty(filter.ToClave))
                    where += " AND Detalle_Probable_Responsable_de_Documentos.Clave <= " + filter.ToClave;
            }
            ...
            if (!string.IsNullOrEmpty(filter.IdDocumentos))
            {
                switch (filter.IdDocumentosFilter)
                {
                    case Models.Filters.BeginWith:
                        where += " AND Detalle_Probable_Responsable_de_Documentos.IdDocumentos LIKE '" + filter.IdDocumentos + "%'";
                        break;
                    ...
                    case Models.Filters.Exact:
                        where += " AND Detalle_Probable_Responsable_de_Documentos.IdDocumentos = '" + filter.IdDocumentos + "'";
                        break;
            ...
            else if (filter.AdvanceProbable_ResponsableMultiple != null && filter.AdvanceProbable_ResponsableMultiple.Count() > 0)
            {
                ...
                where += " AND Detalle_Probable_Responsable_de_Documentos.Probable_Responsable In (" + Probable_ResponsableIds + ")";
            }
            ...
            try
            {
				...
				//{
                    ...
                    var Detalle_Probable_Responsable_de_DocumentosInfo = new Detalle_Probable_Responsable_de_Documentos
                    {
                        ...
                        ,IdDocumentos = varDetalle_Probable_Responsable_de_Documentos.IdDocumentos
        ...
        /// <returns></returns>
        [HttpPost]
        public JsonResult WriteScriptSettings(CustomElementsNew Detalle_Probable_Responsable_de_DocumentosModuleAttributeList)
        {
			...
			if (Detalle_Probable_Responsable_de_DocumentosModuleAttributeList.ChildModuleAttributeList != null)
            {
				...
				{
					for (int j = 0; j < Detalle_Probable_Responsable_de_DocumentosModuleAttributeList.ChildModuleAttributeList[i].elements.Count; j++)
					{
						if (string.IsNullOrEmpty(Detalle_Probable_Responsable_de_DocumentosModuleAttributeList.ChildModuleAttributeList[i].elements[j].DefaultValue))
						{
							Detalle_Probable_Responsable_de_DocumentosModuleAttributeList.ChildModuleAttributeList[i].elements[j].DefaultValue = string.Empty;
						}
						if (string.IsNullOrEmpty(Detalle_Probable_Responsable_de_DocumentosModuleAttributeList.ChildModuleAttributeList[i].elements[j].HelpText))
						{
							Detalle_Probable_Responsable_de_DocumentosModuleAttributeList.ChildModuleAttributeList[i].elements[j].HelpText = string.Empty;
						}
					}
				}
			}
			...
			if (Detalle_Probable_Responsable_de_DocumentosModuleAttributeList.ChildModuleAttributeList != null)
            {
				indexOfArrayHistory = strDetalle_Probable_Responsable_de_DocumentosScript.IndexOf("});");
		   ...
		   var ResponseChild = string.Empty;
            if (Detalle_Probable_Responsable_de_DocumentosModuleAttributeList.ChildModuleAttributeList != null && indexOfArrayHistory != -1)
            {
                foreach (var item in Detalle_Probable_Responsable_de_DocumentosModuleAttributeList.ChildModuleAttributeList)
                {
        ...
        public ActionResult GetFormats(string RecordId)
        {
           ...
           _ISpartan_FormatApiConsumer.SetAuthHeader(_tokenManager.Token);
            ...
            if (formatsPermisions.RowCount > 0)
            {
                ...
                var whereClauseFormat = "Object = 45389 AND FormatId in (" + formats + ")";
                ...
                if (Spartan_Formats != null && Spartan_Formats.Resource != null && Spartan_Formats.Resource.Spartan_Formats != null)
                {
                    ...
                    foreach (Spartan_Format format in Spartan_Formats.Resource.Spartan_Formats)
                    {
                        ...
                        {
                            var where = Helper.ReplaceGlobal(format.Filter).Trim() + " And Detalle_Probable_Responsable_de_Documentos.Clave= " + RecordId;
                            var result = _IDetalle_Probable_Responsable_de_DocumentosApiConsumer.ListaSelAll(0, 1000, where, string.Empty);
        ...
        public void Export(string format, int pageIndex, int pageSize, string iSortCol, string sSortDir, string where, string order, dynamic columnsVisible)
        {
            ...
            var configuration = new GridConfiguration() { OrderByClause = "", WhereClause = "" };
            if (filter != null)
                configuration = GridQueryHelper.GetDataTableConfiguration(filter, new Detalle_Probable_Responsable_de_DocumentosPropertyMapper());
            ...
            configuration.OrderByClause =  oDetalle_Probable_Responsable_de_DocumentosPropertyMapper.GetPropertyName(iSortCol)  + " " + sortDirection;
            ...
            switch (exportFormatType)
            {
                case ExportFormatType.PDF:
                    PdfConverter.ExportToPdf(data.ToDataTable(45389, arrayColumnsVisible), "Detalle_Probable_Responsable_de_DocumentosList_" + DateTime.Now.ToString());
                    break;

                case ExportFormatType.EXCEL:
                    ExcelConverter.ExportToExcel(data.ToDataTable(45389, arrayColumnsVisible), "Detalle_Probable_Responsable_de_DocumentosList_" + DateTime.Now.ToString());
                    break;

                case ExportFormatType.CSV:
                    CsvConverter.ExportToCSV(data.ToDataTable(45389, arrayColumnsVisible), "Detalle_Probable_Responsable_de_DocumentosList_" + DateTime.Now.ToString());
                    break;
            }
        }
        ...
        public JsonResult CreateID()
        {
            try
            {
                if (!_tokenManager.GenerateToken())
                    return Json(null, JsonRequestBehavior.AllowGet);
                _IDetalle_Probable_Responsable_de_DocumentosApiConsumer.SetAuthHeader(_tokenManager.Token);
                var result = _IDetalle_Probable_Responsable_de_DocumentosApiConsumer.GenerateID().Resource.ToString();
                Session["KeyValueInserted"] = result;
                return Json(result, JsonRequestBehavior.AllowGet);
            }
            catch (ServiceException ex)
		...
		[HttpPost]
        public ActionResult Post_Datos_Generales(Detalle_Probable_Responsable_de_Documentos_Datos_GeneralesModel varDetalle_Probable_Responsable_de_Documentos)
        {
            try
            {
                if (!_tokenManager.GenerateToken())
                ...
                var result = "";
                var Detalle_Probable_Responsable_de_Documentos_Datos_GeneralesInfo = new Detalle_Probable_Responsable_de_Documentos_Datos_Generales
                {
                    Clave = varDetalle_Probable_Responsable_de_Documentos.Clave
                                            ,IdDocumentos = varDetalle_Probable_Responsable_de_Documentos.IdDocumentos
                        ,Probable_Responsable = varDetalle_Probable_Responsable_de_Documentos.Probable_Responsable
                ...
                result = _IDetalle_Probable_Responsable_de_DocumentosApiConsumer.Update_Datos_Generales(Detalle_Probable_Responsable_de_Documentos_Datos_GeneralesInfo).Resource.ToString();
                Session["KeyValueInserted"] = result;
                return Json(result, JsonRequestBehavior.AllowGet);
            }
            catch (ServiceException ex)
        ...
        {     
            ...
            {                
                if (!_tokenManager.GenerateToken())
                    return Json(null, JsonRequestBehavior.AllowGet);
                _IDetalle_Probable_Responsable_de_DocumentosApiConsumer.SetAuthHeader(_tokenManager.Token);
                var m = _IDetalle_Probable_Responsable_de_DocumentosApiConsumer.Get_Datos_Generales(Id).Resource;
                if (m == null)
                    return Json(null, JsonRequestBehavior.AllowGet);
				
                var result = new Detalle_Probable_Responsable_de_Documentos_Datos_GeneralesModel
                {
                    Clave = m.Clave
			,IdDocumentos = m.IdDocumentos
