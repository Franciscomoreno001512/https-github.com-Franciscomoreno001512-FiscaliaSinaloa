...
$(document).ready(function () {
    ...
    $("#Modelo").keyup(function () {
        var str = $('#' + nameOfTable + 'Modelo' + rowIndex).val();
        if (str != "") {
            var regNUM = /^[0-9]{1,4}$/
            var validacionnum = regNUM.test(str)
            if (validacionnum == false) {
                $('#' + nameOfTable + 'Modelo' + rowIndex).val($('#' + nameOfTable + 'Modelo' + rowIndex).val().slice(0, -1));
            }
        }
    });

    if (operation == 'Update') {
		if (EvaluaQuery("select Canalizado from Detalle_de_Delito where Clave = FLDD[lblClave]",rowIndex, nameOfTable)==TryParseInt('1', '1') && TryParseInt(ReplaceGLOBAL('GLOBAL[idTablero]'), ReplaceGLOBAL('GLOBAL[idTablero]'))==TryParseInt('2', '2')) {
...
//BusinessRuleId:4576, Attribute:263447, Operation:Field, Event:None
$("form#CreateDetalle_de_Delito").on('change', '#Delito', function () {
	nameOfTable='';
	rowIndex='';
if( GetValueByControlType($('#' + nameOfTable + 'Delito' + rowIndex),nameOfTable,rowIndex)!=TryParseInt('null', 'null') ) { 
    var varTitulo = EvaluaQuery(" select clave from Titulo_del_Delito where clave = ( select titulo_del_delito from delito where clave = FLD[Delito])", rowIndex, nameOfTable);// AsignarValor($('#' + nameOfTable + 'Titulo_del_Delito' + rowIndex),EvaluaQuery(" select Descripcion from Titulo_del_Delito where clave = ( select titulo_del_delito from delito where clave = FLD[Delito])", rowIndex, nameOfTable)); 
    var varGrupo = EvaluaQuery(" select Clave from Grupo_del_Delito where clave = ( select Grupo_del_Delito from delito where clave = FLD[Delito])", rowIndex, nameOfTable);// AsignarValor($('#' + nameOfTable + 'Grupo_Delito' + rowIndex),EvaluaQuery(" select  Descripcion from Grupo_del_Delito where clave = ( select Grupo_del_Delito from delito where clave = FLD[Delito])", rowIndex, nameOfTable));
 var valor =$('#' + nameOfTable + 'Titulo_del_Delito' + rowIndex).val();
    $('#' + nameOfTable + 'Titulo_del_Delito' + rowIndex).empty();
    if(!$('#' + nameOfTable + 'Titulo_del_Delito' + rowIndex).hasClass('AutoComplete'))  
    {        
        $('#' + nameOfTable + 'Titulo_del_Delito' + rowIndex).append($("<option selected />").val("").text(""));  
        $.each(EvaluaQueryDictionary("select clave,Descripcion from Titulo_del_Delito where clave = ( select titulo_del_delito from delito where clave = FLD[Delito])", rowIndex, nameOfTable), function (index, value) {
        $('#' + nameOfTable + 'Titulo_del_Delito' + rowIndex).append($("<option />").val(index).text(value));      });  
    }       
    else {   
        var selectData = [];   selectData.push({id: "",text: "" });     
        $.each(EvaluaQueryDictionary("select clave,Descripcion from Titulo_del_Delito where clave = ( select titulo_del_delito from delito where clave = FLD[Delito])", rowIndex, nameOfTable), function (index, value) {      
        selectData.push({              id: index,              text: value          });    });      $('#' + nameOfTable + 'Titulo_del_Delito' + rowIndex).select2({data: selectData})    
    }  
    $('#' + nameOfTable + 'Titulo_del_Delito' + rowIndex).val(varTitulo).trigger('change');

    var valor = $('#' + nameOfTable + 'Grupo_Delito' + rowIndex).val();  
     $('#' + nameOfTable + 'Grupo_Delito' + rowIndex).empty();         
     if(!$('#' + nameOfTable + 'Grupo_Delito' + rowIndex).hasClass('AutoComplete'))  
     {         
         $('#' + nameOfTable + 'Grupo_Delito' + rowIndex).append($("<option selected />").val("").text(""));         
         $.each(EvaluaQueryDictionary("select Clave, Descripcion from Grupo_del_Delito where clave = ( select Grupo_del_Delito from delito where clave = FLD[Delito])", rowIndex, nameOfTable), function (index, value) 
         {          
              $('#' + nameOfTable + 'Grupo_Delito' + rowIndex).append($("<option />").val(index).text(value));      });  
    }       
    else    
    {    
        var selectData = [];   selectData.push({id: "",text: "" });      
        ...
        $('#' + nameOfTable + 'Grupo_Delito' + rowIndex).select2({data: selectData})    }  
         $('#' + nameOfTable + 'Grupo_Delito' + rowIndex).val(varGrupo).trigger('change'); 
...
//BusinessRuleId:4704, Attribute:263447, Operation:Field, Event:None
$("form#CreateDetalle_de_Delito").on('change', '#Delito', function () {
	nameOfTable='';
	rowIndex='';
if( TryParseInt(ReplaceGLOBAL('GLOBAL[idTablero]'), ReplaceGLOBAL('GLOBAL[idTablero]'))==TryParseInt('2', '2') && EvaluaQuery("select ISNULL(Robo_de_Vehiculo, 0) from Delito where Clave = FLD[Delito]",rowIndex, nameOfTable)==TryParseInt('0', '0') ) { $("a[href='#tabDatos_de_Robo_de_Vehiculo']").css('display', 'none'); $("a[href='#tabAsignar_Dueno']").css('display', 'none');} else { $("a[href='#tabAsignar_Dueno']").css('display', 'block'); $("a[href='#tabDatos_de_Robo_de_Vehiculo']").css('display', 'block');}
});
...
//BusinessRuleId:4701, Attribute:2, Operation:Object, Event:BEFORESAVING
if(operation == 'Update'){
if( EvaluaQuery("select COUNT(*) from Detalle_de_Delito where Expediente_Inicial = FLD[Expediente_Inicial] and Delito = FLD[Delito] and clave != FLDD[lblClave]",rowIndex, nameOfTable)>TryParseInt('0', '0') ) { alert(DecodifyText(' No se puede agregar el mismo delito en el expediente.', rowIndex, nameOfTable));
...
//BusinessRuleId:4702, Attribute:2, Operation:Object, Event:BEFORESAVING
if(operation == 'New'){
if( EvaluaQuery("select COUNT(*) from Detalle_de_Delito where Expediente_Inicial = FLD[Expediente_Inicial] and Delito = FLD[Delito] and clave != GLOBAL[keyvalueinserted]",rowIndex, nameOfTable)>TryParseInt('0', '0') ) { alert(DecodifyText(' No se puede agregar el mismo delito al expediente.', rowIndex, nameOfTable));
...
//BusinessRuleId:4739, Attribute:2, Operation:Object, Event:BEFORESAVING
if(operation == 'New'){
if( EvaluaQuery("select COUNT(*) from Detalle_de_Delito  where Expediente_Inicial =GLOBAL[SpartanOperationId]",rowIndex, nameOfTable)==TryParseInt('0', '0') ) { AsignarValor($('#' + nameOfTable + 'Tipo_de_Delito' + rowIndex),'1');} else {}
...
//BusinessRuleId:4741, Attribute:2, Operation:Object, Event:BEFORESAVING
if(operation == 'New'){
    debugger;
if( EvaluaQuery("select COUNT(*) from Detalle_de_Delito where Expediente_Inicial = GLOBAL[SpartanOperationId]",rowIndex, nameOfTable)>TryParseInt('1', '1') && 
GetValueByControlType($('#' + nameOfTable + 'Tipo_de_Delito' + rowIndex),nameOfTable,rowIndex)==TryParseInt('1', '1') ) { 
    EvaluaQuery(" exec uspUpdateDelitoPrincipal GLOBAL[SpartanOperationId], GLOBAL[keyvalueinserted]", rowIndex, nameOfTable);} 
    else {}
...
//BusinessRuleId:4742, Attribute:2, Operation:Object, Event:BEFORESAVING
if(operation == 'Update'){
if( EvaluaQuery("select COUNT(*) from Detalle_de_Delito where Expediente_Inicial = GLOBAL[SpartanOperationId]",rowIndex, nameOfTable)>TryParseInt('1', '1') && GetValueByControlType($('#' + nameOfTable + 'Tipo_de_Delito' + rowIndex),nameOfTable,rowIndex)==TryParseInt('1', '1') ) { EvaluaQuery(" exec uspUpdateDelitoPrincipal GLOBAL[SpartanOperationId], FLDD[lblClave]", rowIndex, nameOfTable);} else {}

}
//BusinessRuleId:4742, Attribute:2, Operation:Object, Event:BEFORESAVING

//BusinessRuleId:4743, Attribute:2, Operation:Object, Event:BEFORESAVING
if(operation == 'Update'){
if( EvaluaQuery("select COUNT(*) from Detalle_de_Delito where Expediente_Inicial = GLOBAL[SpartanOperationId]"
+" and tipo_de_delito = 1 and Clave!=  FLDD[lblClave]",rowIndex, nameOfTable)==TryParseInt('0', '0') && GetValueByControlType($('#' + nameOfTable + 'Tipo_de_Delito' + rowIndex),nameOfTable,rowIndex)!=TryParseInt('1', '1') ) { alert(DecodifyText(' No se puede cambiar el tipo de delito a Secundario, cambie el registro que necesite sea Delito Principal', rowIndex, nameOfTable));
result=false;} else {}

}
//BusinessRuleId:4743, Attribute:2, Operation:Object, Event:BEFORESAVING

//BusinessRuleId:4798, Attribute:2, Operation:Object, Event:BEFORESAVING
if(operation == 'New'){
if( EvaluaQuery("select COUNT(*) from Detalle_de_Delito where Expediente_Inicial = GLOBAL[SpartanOperationId]",rowIndex, nameOfTable)>TryParseInt('1', '1') && GetValueByControlType($('#' + nameOfTable + 'Tipo_de_Delito' + rowIndex),nameOfTable,rowIndex)==TryParseInt('null', 'null') ) { AsignarValor($('#' + nameOfTable + 'Tipo_de_Delito' + rowIndex),'2');} else {}
...
//BusinessRuleId:4934, Attribute:2, Operation:Object, Event:AFTERSAVING
if(operation == 'New'){
if( TryParseInt(ReplaceGLOBAL('GLOBAL[idTablero]'), ReplaceGLOBAL('GLOBAL[idTablero]'))==TryParseInt('6', '6') ) { EvaluaQuery("update detalle_de_delito set Mandamiento_Judicial = GLOBAL[SpartanOperationId] where Clave=GLOBAL[keyvalueinserted]", rowIndex, nameOfTable);} else {}
