...
$(document).ready(function() {
    ...
    if (operation == 'Update') {
        if (EvaluaQuery("select Canalizado from Detalle_de_Imputado where Clave = FLDD[lblClave]", rowIndex, nameOfTable) == TryParseInt('1', '1') && TryParseInt(ReplaceGLOBAL('GLOBAL[idTablero]'), ReplaceGLOBAL('GLOBAL[idTablero]')) == TryParseInt('2', '2')) {
            $("#Detalle_de_ImputadoGuardar").hide();
        }
    }
    if (operation == 'Update') {
    ...
    $("#Alias").bind('keypress', function(event) {
        var regex = new RegExp("^[a-zA-Z 0-9 ñÑ]+$");
    ...
    $("form#CreateDetalle_de_Imputado").on('change', '#Inimputable', function() {
        nameOfTable = '';
        rowIndex = '';
        if (EvaluaQuery("select 'FLD[Inimputable]'", rowIndex, nameOfTable) == TryParseInt('true', 'true')) {
            $('#divTipo_de_Inimputabilidad').css('display', 'block');
            $('#divEspecifique').css('display', 'block');
            $("a[href='#tabDatos_del_Tutor']").css('display', 'block');
    ...
    $("form#CreateDetalle_de_Imputado").on('change', '#Bajo_el_Efecto_de_una_Droga', function() {
        nameOfTable = '';
        rowIndex = '';
        if (GetValueByControlType($('#' + nameOfTable + 'Bajo_el_Efecto_de_una_Droga' + rowIndex), nameOfTable, rowIndex) == TryParseInt('true', 'true')) {
            $('#divNombre_de_Droga').css('display', 'block');
            SetRequiredToControl($('#' + nameOfTable + 'Nombre_de_Droga' + rowIndex));
        } else {
    ...
    $("form#CreateDetalle_de_Imputado").on('change', '#Escolaridad_Detenido', function() {
        nameOfTable = '';
        rowIndex = '';
        if (GetValueByControlType($('#' + nameOfTable + 'Escolaridad_Detenido' + rowIndex), nameOfTable, rowIndex) == TryParseInt('6', '6')) {
    ...
    $("form#CreateDetalle_de_Imputado").on('change', '#Pais_del_Tutor', function() {
        nameOfTable = '';
        rowIndex = '';
        if (GetValueByControlType($('#' + nameOfTable + 'Pais_del_Tutor' + rowIndex), nameOfTable, rowIndex) != TryParseInt('null', 'null') && GetValueByControlType($('#' + nameOfTable + 'Pais_del_Tutor' + rowIndex), nameOfTable, rowIndex) == TryParseInt('82', '82')) {
            ...
            $('#divColonia_del_Tutor').css('display', 'block');
        } else {}
    });
    ...
    $("form#CreateDetalle_de_Imputado").on('change', '#Municipio_del_Tutor', function() {
        ...
        if (GetValueByControlType($('#' + nameOfTable + 'Municipio_del_Tutor' + rowIndex), nameOfTable, rowIndex) != TryParseInt('null', 'null')) {
            ...
            if (!$('#' + nameOfTable + 'Poblacion_Tutor' + rowIndex).hasClass('AutoComplete')) {
                ...
                $.each(EvaluaQueryDictionary("SELECT Clave, Nombre FROM Colonia WHERE Municipio = FLD[Municipio_del_Tutor] ", rowIndex, nameOfTable), function(index, value) { $('#' + nameOfTable + 'Poblacion_Tutor' + rowIndex).append($("<option />").val(index).text(value)); });
            } else {
                var selectData = [];
                selectData.push({ id: "", text: "" });
                $.each(EvaluaQueryDictionary("SELECT Clave, Nombre FROM Colonia WHERE Municipio = FLD[Municipio_del_Tutor] ", rowIndex, nameOfTable), function(index, value) { selectData.push({ id: index, text: value }); });
            ...
            if (!$('#' + nameOfTable + 'Colonia_del_Tutor' + rowIndex).hasClass('AutoComplete')) {
                ...
                $.each(EvaluaQueryDictionary("SELECT Clave, Nombre FROM Colonia WHERE Municipio = FLD[Municipio_del_Tutor] ", rowIndex, nameOfTable), function(index, value) { $('#' + nameOfTable + 'Colonia_del_Tutor' + rowIndex).append($("<option />").val(index).text(value)); });
            } else {
                var selectData = [];
                selectData.push({ id: "", text: "" });
                $.each(EvaluaQueryDictionary("SELECT Clave, Nombre FROM Colonia WHERE Municipio = FLD[Municipio_del_Tutor] ", rowIndex, nameOfTable), function(index, value) { selectData.push({ id: index, text: value }); });
    ...
    $("form#CreateDetalle_de_Imputado").on('change', '#Poblacion_Tutor', function() {
        ...
        if (GetValueByControlType($('#' + nameOfTable + 'Poblacion_Tutor' + rowIndex), nameOfTable, rowIndex) != TryParseInt('null', 'null')) {
            ...
            if (!$('#' + nameOfTable + 'Colonia_del_Tutor' + rowIndex).hasClass('AutoComplete')) {
                $('#' + nameOfTable + 'Colonia_del_Tutor' + rowIndex).append($("<option selected />").val("").text(""));
                $.each(EvaluaQueryDictionary("SELECT Clave, Nombre FROM Colonia WHERE Poblacion = FLD[Poblacion_Tutor] ", rowIndex, nameOfTable), function(index, value) { $('#' + nameOfTable + 'Colonia_del_Tutor' + rowIndex).append($("<option />").val(index).text(value)); });
            } else {
                var selectData = [];
                selectData.push({ id: "", text: "" });
                $.each(EvaluaQueryDictionary("SELECT Clave, Nombre FROM Colonia WHERE Poblacion = FLD[Poblacion_Tutor] ", rowIndex, nameOfTable), function(index, value) { selectData.push({ id: index, text: value }); });
                $('#' + nameOfTable + 'Colonia_del_Tutor' + rowIndex).select2({ data: selectData })
            }
            $('#' + nameOfTable + 'Colonia_del_Tutor' + rowIndex).val(valor).trigger('change');
        } else {
    ...
    //OCULTAR/MOSTRAR TAGS DEPENDIENDO DE CAMPO QUIEN RESULTE RESPONSABLE - FELIPE RDZ
    $("form#CreateDetalle_de_Imputado").on('change', '#Quien_Resulte_Responsable', function() {
        nameOfTable = '';
        rowIndex = '';
        if (GetValueByControlType($('#' + nameOfTable + 'Quien_Resulte_Responsable' + rowIndex), nameOfTable, rowIndex) == TryParseInt('true', 'true')) {
            $('#divDatos_DireccionActual').css('display', 'none');
            $('#divDatos_Origen').css('display', 'none');
            $('#divDatos_Personales').css('display', 'none');
            $('#divDiscapacidades').css('display', 'none');
        } else {
            $('#divDatos_DireccionActual').css('display', 'block');
            $('#divDatos_Origen').css('display', 'block');
            $('#divDatos_Personales').css('display', 'block');
            $('#divDiscapacidades').css('display', 'block');
        }
    });
...
//BusinessRuleId:4750, Attribute:273416, Operation:Field, Event:None
$("form#CreateDetalle_de_Imputado").on('change', '#Suspencion_Condicional', function () {
	nameOfTable='';
	rowIndex='';
if( GetValueByControlType($('#' + nameOfTable + 'Suspencion_Condicional' + rowIndex),nameOfTable,rowIndex)==TryParseInt('true', 'true') ) { $('#divFecha_Suspencion_Condicional').css('display', 'block');} else { $('#divFecha_Suspencion_Condicional').css('display', 'none'); SetNotRequiredToControl( $('#' + nameOfTable + 'Fecha_Suspencion_Condicional' + rowIndex));}
});
...
//BusinessRuleId:4749, Attribute:273412, Operation:Field, Event:None
$("form#CreateDetalle_de_Imputado").on('change', '#Situacion_del_Imputado', function () {
	nameOfTable='';
	rowIndex='';
if( GetValueByControlType($('#' + nameOfTable + 'Situacion_del_Imputado' + rowIndex),nameOfTable,rowIndex)==TryParseInt('1', '1') ) { $('#divFecha_de_Detencion').css('display', 'block');$('#divMunicipio_Detencion').css('display', 'block');$('#divCoorporacion').css('display', 'block');$('#divSuspencion_Condicional').css('display', 'block');} else { $('#divFecha_de_Detencion').css('display', 'none'); SetNotRequiredToControl( $('#' + nameOfTable + 'Fecha_de_Detencion' + rowIndex));$('#divMunicipio_Detencion').css('display', 'none'); SetNotRequiredToControl( $('#' + nameOfTable + 'Municipio_Detencion' + rowIndex));$('#divCoorporacion').css('display', 'none'); SetNotRequiredToControl( $('#' + nameOfTable + 'Coorporacion' + rowIndex));$('#divSuspencion_Condicional').css('display', 'none'); SetNotRequiredToControl( $('#' + nameOfTable + 'Suspencion_Condicional' + rowIndex));$('#divFecha_Suspencion_Condicional').css('display', 'none'); SetNotRequiredToControl( $('#' + nameOfTable + 'Fecha_Suspencion_Condicional' + rowIndex));}
});
...
//BusinessRuleId:4763, Attribute:265784, Operation:Field, Event:None
$("form#CreateDetalle_de_Imputado").on('change', '#Se_Presenta_con_Detenido', function () {
	nameOfTable='';
	rowIndex='';
if( GetValueByControlType($('#' + nameOfTable + 'Se_Presenta_con_Detenido' + rowIndex),nameOfTable,rowIndex)==TryParseInt('true', 'true') ) { if('false' == 'true')
{
...
} AsignarValor($('#' + nameOfTable + 'Occiso' + rowIndex),'false');} else { if('true' == 'true')
{
	$('#divLugar_de_Detencion').css('display', 'none');
}
else
...
//BusinessRuleId:4766, Attribute:273418, Operation:Field, Event:None
$("form#CreateDetalle_de_Imputado").on('change', '#Occiso', function () {
	nameOfTable='';
	rowIndex='';
if( GetValueByControlType($('#' + nameOfTable + 'Occiso' + rowIndex),nameOfTable,rowIndex)==TryParseInt('true', 'true') ) { AsignarValor($('#' + nameOfTable + 'Se_Presenta_con_Detenido' + rowIndex),'false');} else {}
});


//BusinessRuleId:4766, Attribute:273418, Operation:Field, Event:None

//BusinessRuleId:4795, Attribute:263187, Operation:Field, Event:None
$("form#CreateDetalle_de_Imputado").on('change', '#Quien_Resulte_Responsable', function () {
	nameOfTable='';
	rowIndex='';
if( GetValueByControlType($('#' + nameOfTable + 'Quien_Resulte_Responsable' + rowIndex),nameOfTable,rowIndex)==TryParseInt('true', 'true') ) { 
    AsignarValor($('#' + nameOfTable + 'Nombre' + rowIndex),'QUIEN O QUIENES'); 
    AsignarValor($('#' + nameOfTable + 'Apellido_Paterno' + rowIndex),'RESULTEN');
     AsignarValor($('#' + nameOfTable + 'Apellido_Materno' + rowIndex),'RESPONSABLES');} 
     else { 
         AsignarValor($('#' + nameOfTable + 'Nombre' + rowIndex),'');
         AsignarValor($('#' + nameOfTable + 'Apellido_Paterno' + rowIndex),'');
         AsignarValor($('#' + nameOfTable + 'Apellido_Materno' + rowIndex),'');}
});
...
function EjecutarValidacionesAlComienzo() {
    ...
    if (operation == 'New') {
        if (GetValueByControlType($('#' + nameOfTable + 'Inimputable' + rowIndex), nameOfTable, rowIndex) == TryParseInt('true', 'true')) {
            $('#divTipo_de_Inimputabilidad').css('display', 'block');
            $('#divEspecifique').css('display', 'block');
            $("a[href='#tabDatos_del_Tutor']").css('display', 'block');
            SetRequiredToControl($('#' + nameOfTable + 'Tipo_de_Inimputabilidad' + rowIndex));
    ...
    //OCULTAR/MOSTRAR TAGS DEPENDIENDO DE CAMPO QUIEN RESULTE RESPONSABLE AL EDITAR Y CONSULTAR - FELIPE RDZ
    if (operation == 'Update') {
        if (GetValueByControlType($('#' + nameOfTable + 'Quien_Resulte_Responsable' + rowIndex), nameOfTable, rowIndex) == TryParseInt('true', 'true')) {
            $('#divDatos_DireccionActual').css('display', 'none');
            $('#divDatos_Origen').css('display', 'none');
    ...
    //BusinessRuleId:4378, Attribute:0, Operation:Object, Event:SCREENOPENING
if(operation == 'Update'){
if( TryParseInt('107', '107')==EvaluaQuery("SELECT GLOBAL[USERROLEID]",rowIndex, nameOfTable) ) { DisabledControl($("#" + nameOfTable + "Clave" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Clave' + rowIndex));}DisabledControl($("#" + nameOfTable + "Nombre" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Nombre' + rowIndex));}DisabledControl($("#" + nameOfTable + "Apellido_Paterno" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Apellido_Paterno' + rowIndex));}DisabledControl($("#" + nameOfTable + "Apellido_Materno" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Apellido_Materno' + rowIndex));}DisabledControl($("#" + nameOfTable + "Fecha_de_Nacimiento" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Fecha_de_Nacimiento' + rowIndex));}DisabledControl($("#" + nameOfTable + "Edad" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Edad' + rowIndex));}DisabledControl($("#" + nameOfTable + "Sexo" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Sexo' + rowIndex));}DisabledControl($("#" + nameOfTable + "Estado_Civil" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Estado_Civil' + rowIndex));}DisabledControl($("#" + nameOfTable + "Tipo_de_Identificacion" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Tipo_de_Identificacion' + rowIndex));}DisabledControl($("#" + nameOfTable + "Numero_de_Identificacion" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Numero_de_Identificacion' + rowIndex));}DisabledControl($("#" + nameOfTable + "Nacionalidad" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Nacionalidad' + rowIndex));}DisabledControl($("#" + nameOfTable + "Escolaridad" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Escolaridad' + rowIndex));}DisabledControl($("#" + nameOfTable + "Ocupacion" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Ocupacion' + rowIndex));}DisabledControl($("#" + nameOfTable + "Municipio" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Municipio' + rowIndex));}DisabledControl($("#" + nameOfTable + "Codigo_Postal" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Codigo_Postal' + rowIndex));}DisabledControl($("#" + nameOfTable + "Colonia" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Colonia' + rowIndex));}DisabledControl($("#" + nameOfTable + "Calle_del_Imputado" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Calle_del_Imputado' + rowIndex));}DisabledControl($("#" + nameOfTable + "Numero_Exterior" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Numero_Exterior' + rowIndex));}DisabledControl($("#" + nameOfTable + "Numero_Interior" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Numero_Interior' + rowIndex));}DisabledControl($("#" + nameOfTable + "Telefono" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Telefono' + rowIndex));}DisabledControl($("#" + nameOfTable + "Extension" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Extension' + rowIndex));}DisabledControl($("#" + nameOfTable + "Celular" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Celular' + rowIndex));}DisabledControl($("#" + nameOfTable + "Correo_Electronico" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Correo_Electronico' + rowIndex));}DisabledControl($("#" + nameOfTable + "Pais" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Pais' + rowIndex));}DisabledControl($("#" + nameOfTable + "Forma_Cara" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Forma_Cara' + rowIndex));}DisabledControl($("#" + nameOfTable + "Cejas" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Cejas' + rowIndex));}DisabledControl($("#" + nameOfTable + "Cantidad_Cabello" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Cantidad_Cabello' + rowIndex));}DisabledControl($("#" + nameOfTable + "Implantacion_Cabello" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Implantacion_Cabello' + rowIndex));}DisabledControl($("#" + nameOfTable + "Complexion" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Complexion' + rowIndex));}DisabledControl($("#" + nameOfTable + "Color_Piel" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Color_Piel' + rowIndex));}DisabledControl($("#" + nameOfTable + "Frente" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Frente' + rowIndex));}DisabledControl($("#" + nameOfTable + "Forma_Cabello" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Forma_Cabello' + rowIndex));}DisabledControl($("#" + nameOfTable + "Color_Cabello" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Color_Cabello' + rowIndex));}DisabledControl($("#" + nameOfTable + "Calvicie" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Calvicie' + rowIndex));}DisabledControl($("#" + nameOfTable + "Color_Ojos" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Color_Ojos' + rowIndex));}DisabledControl($("#" + nameOfTable + "Forma_Ojos" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Forma_Ojos' + rowIndex));}DisabledControl($("#" + nameOfTable + "Nariz_Base" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Nariz_Base' + rowIndex));}DisabledControl($("#" + nameOfTable + "Labios" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Labios' + rowIndex));}DisabledControl($("#" + nameOfTable + "Boca" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Boca' + rowIndex));}DisabledControl($("#" + nameOfTable + "Menton" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Menton' + rowIndex));}DisabledControl($("#" + nameOfTable + "Barba" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Barba' + rowIndex));}DisabledControl($("#" + nameOfTable + "Forma_Orejas" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Forma_Orejas' + rowIndex));}DisabledControl($("#" + nameOfTable + "Tamano_Orejas" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Tamano_Orejas' + rowIndex));}DisabledControl($("#" + nameOfTable + "Tipo_Lobulo" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Tipo_Lobulo' + rowIndex));}DisabledControl($("#" + nameOfTable + "Bigote" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Bigote' + rowIndex));}DisabledControl($("#" + nameOfTable + "Situacion_Fisica" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Situacion_Fisica' + rowIndex));}DisabledControl($("#" + nameOfTable + "Alias" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Alias' + rowIndex));}DisabledControl($("#" + nameOfTable + "Quien_Resulte_Responsable" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Quien_Resulte_Responsable' + rowIndex));}DisabledControl($("#" + nameOfTable + "Pais_de_Nacimiento" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Pais_de_Nacimiento' + rowIndex));}DisabledControl($("#" + nameOfTable + "Originario_de" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Originario_de' + rowIndex));}DisabledControl($("#" + nameOfTable + "Referencia_de_Domicilio" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Referencia_de_Domicilio' + rowIndex));}DisabledControl($("#" + nameOfTable + "Narrativa_Breve_de_los_Hechos" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Narrativa_Breve_de_los_Hechos' + rowIndex));}DisabledControl($("#" + nameOfTable + "Prioridad_del_Hecho" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Prioridad_del_Hecho' + rowIndex));}DisabledControl($("#" + nameOfTable + "Fecha_del_Hecho" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Fecha_del_Hecho' + rowIndex));}DisabledControl($("#" + nameOfTable + "Hora_Aproximada_del_Hecho" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Hora_Aproximada_del_Hecho' + rowIndex));}DisabledControl($("#" + nameOfTable + "Pais_del_hecho" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Pais_del_hecho' + rowIndex));}DisabledControl($("#" + nameOfTable + "Estado_del_Hecho" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Estado_del_Hecho' + rowIndex));}DisabledControl($("#" + nameOfTable + "Municipio_del_Hecho" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Municipio_del_Hecho' + rowIndex));}DisabledControl($("#" + nameOfTable + "Colonia_del_hecho" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Colonia_del_hecho' + rowIndex));}DisabledControl($("#" + nameOfTable + "Codigo_Postal_del_hecho" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Codigo_Postal_del_hecho' + rowIndex));}DisabledControl($("#" + nameOfTable + "Calle_del_hecho" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Calle_del_hecho' + rowIndex));}DisabledControl($("#" + nameOfTable + "Entre_Calle" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Entre_Calle' + rowIndex));}DisabledControl($("#" + nameOfTable + "Y_Calle" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Y_Calle' + rowIndex));}DisabledControl($("#" + nameOfTable + "Numero_Exterior_de_los_Hechos" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Numero_Exterior_de_los_Hechos' + rowIndex));}DisabledControl($("#" + nameOfTable + "Numero_interior_de_los_hecho" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Numero_interior_de_los_hecho' + rowIndex));}DisabledControl($("#" + nameOfTable + "Latitud_de_los_hechos" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Latitud_de_los_hechos' + rowIndex));}DisabledControl($("#" + nameOfTable + "Longitud_de_los_Hechos" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Longitud_de_los_Hechos' + rowIndex));}DisabledControl($("#" + nameOfTable + "Tipo_del_lugar_del_he" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Tipo_del_lugar_del_he' + rowIndex));}DisabledControl($("#" + nameOfTable + "Peso" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Peso' + rowIndex));}DisabledControl($("#" + nameOfTable + "Estatura" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Estatura' + rowIndex));}DisabledControl($("#" + nameOfTable + "Inimputable" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Inimputable' + rowIndex));}DisabledControl($("#" + nameOfTable + "Tipo_de_Inimputabilidad" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Tipo_de_Inimputabilidad' + rowIndex));}DisabledControl($("#" + nameOfTable + "Nombre_del_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Nombre_del_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Apellido_Paterno_del_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Apellido_Paterno_del_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Apellido_Materno_del_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Apellido_Materno_del_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Nombre_Completo_del_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Nombre_Completo_del_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Fecha_de_Nacimiento_del_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Fecha_de_Nacimiento_del_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Edad_del_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Edad_del_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Sexo_del_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Sexo_del_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Estado_Civil_del_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Estado_Civil_del_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Tipo_de_Identificacion_del_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Tipo_de_Identificacion_del_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Numero_de_Identificacion_del_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Numero_de_Identificacion_del_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Nacionalidad_del_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Nacionalidad_del_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Escolaridad_del_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Escolaridad_del_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Ocupacion_del_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Ocupacion_del_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Pais_del_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Pais_del_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Estado_del_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Estado_del_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Municipio_del_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Municipio_del_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Colonia_del_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Colonia_del_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Codigo_Postal_del_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Codigo_Postal_del_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Calle_del_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Calle_del_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Numero_Exterior_del_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Numero_Exterior_del_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Numero_Interior_del_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Numero_Interior_del_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Telefono_del_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Telefono_del_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Extension_del_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Extension_del_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Celular_del_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Celular_del_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Correo_Electronico_del_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Correo_Electronico_del_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Pais_de_Origen_tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Pais_de_Origen_tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Originario_de_tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Originario_de_tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "CURP" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'CURP' + rowIndex));}DisabledControl($("#" + nameOfTable + "RFC" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'RFC' + rowIndex));}DisabledControl($("#" + nameOfTable + "CURP_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'CURP_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "RFC_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'RFC_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Probable_Responsable" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Probable_Responsable' + rowIndex));}DisabledControl($("#" + nameOfTable + "Poblacion" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Poblacion' + rowIndex));}DisabledControl($("#" + nameOfTable + "Fecha_de_Detencion" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Fecha_de_Detencion' + rowIndex));}DisabledControl($("#" + nameOfTable + "Municipio_de_Detencion" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Municipio_de_Detencion' + rowIndex));}DisabledControl($("#" + nameOfTable + "Corporacion" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Corporacion' + rowIndex));}DisabledControl($("#" + nameOfTable + "Probable_Resposable" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Probable_Resposable' + rowIndex));}DisabledControl($("#" + nameOfTable + "Tipo_de_Lugar" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Tipo_de_Lugar' + rowIndex));}DisabledControl($("#" + nameOfTable + "Descripcion" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Descripcion' + rowIndex));}DisabledControl($("#" + nameOfTable + "Numero_Telefonico" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Numero_Telefonico' + rowIndex));}DisabledControl($("#" + nameOfTable + "Redes_Sociales" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Redes_Sociales' + rowIndex));}DisabledControl($("#" + nameOfTable + "Observaciones" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Observaciones' + rowIndex));}DisabledControl($("#" + nameOfTable + "Tipo_de_identificacion" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Tipo_de_identificacion' + rowIndex));}DisabledControl($("#" + nameOfTable + "Folio_Adicciones" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Folio_Adicciones' + rowIndex));}DisabledControl($("#" + nameOfTable + "Folio_otros_Nombres" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Folio_otros_Nombres' + rowIndex));}DisabledControl($("#" + nameOfTable + "Suspension_Condicional" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Suspension_Condicional' + rowIndex));}DisabledControl($("#" + nameOfTable + "Fecha_de_Suspension_Condicional" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Fecha_de_Suspension_Condicional' + rowIndex));}DisabledControl($("#" + nameOfTable + "Se_Presenta_con_Detenido" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Se_Presenta_con_Detenido' + rowIndex));}DisabledControl($("#" + nameOfTable + "Folio_Registro_Nacional_de_Detenciones" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Folio_Registro_Nacional_de_Detenciones' + rowIndex));}DisabledControl($("#" + nameOfTable + "Nombre_Completo_Detenido" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Nombre_Completo_Detenido' + rowIndex));}DisabledControl($("#" + nameOfTable + "Apodo" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Apodo' + rowIndex));}DisabledControl($("#" + nameOfTable + "Etnia" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Etnia' + rowIndex));}DisabledControl($("#" + nameOfTable + "No_de_Hijos" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'No_de_Hijos' + rowIndex));}DisabledControl($("#" + nameOfTable + "Religion" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Religion' + rowIndex));}DisabledControl($("#" + nameOfTable + "Servicio_Medico" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Servicio_Medico' + rowIndex));}DisabledControl($("#" + nameOfTable + "Escolaridad_Detenido" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Escolaridad_Detenido' + rowIndex));}DisabledControl($("#" + nameOfTable + "Especialidad" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Especialidad' + rowIndex));}DisabledControl($("#" + nameOfTable + "Estudios_Superiores" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Estudios_Superiores' + rowIndex));}DisabledControl($("#" + nameOfTable + "Idioma" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Idioma' + rowIndex));}DisabledControl($("#" + nameOfTable + "Calidad_Migratoria" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Calidad_Migratoria' + rowIndex));}DisabledControl($("#" + nameOfTable + "Estado_de_Nacimiento" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Estado_de_Nacimiento' + rowIndex));}DisabledControl($("#" + nameOfTable + "Municipio_de_Nacimiento" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Municipio_de_Nacimiento' + rowIndex));}DisabledControl($("#" + nameOfTable + "Dialecto" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Dialecto' + rowIndex));}DisabledControl($("#" + nameOfTable + "Viene_en_Estado_de_Ebriedad" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Viene_en_Estado_de_Ebriedad' + rowIndex));}DisabledControl($("#" + nameOfTable + "Bajo_el_Efecto_de_una_Droga" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Bajo_el_Efecto_de_una_Droga' + rowIndex));}DisabledControl($("#" + nameOfTable + "Nombre_de_Droga" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Nombre_de_Droga' + rowIndex));}DisabledControl($("#" + nameOfTable + "Discapacidad_Mental" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Discapacidad_Mental' + rowIndex));}DisabledControl($("#" + nameOfTable + "Discapacidad_Fisica" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Discapacidad_Fisica' + rowIndex));}DisabledControl($("#" + nameOfTable + "Discapacidad_Sensorial" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Discapacidad_Sensorial' + rowIndex));}DisabledControl($("#" + nameOfTable + "Discapacidad_Psicosocial" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Discapacidad_Psicosocial' + rowIndex));}DisabledControl($("#" + nameOfTable + "Padecimiento_de_Enfermedad" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Padecimiento_de_Enfermedad' + rowIndex));}DisabledControl($("#" + nameOfTable + "Tamano_de_Cejas" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Tamano_de_Cejas' + rowIndex));}DisabledControl($("#" + nameOfTable + "Largo_de_Cabello" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Largo_de_Cabello' + rowIndex));}DisabledControl($("#" + nameOfTable + "Anteojos" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Anteojos' + rowIndex));}DisabledControl($("#" + nameOfTable + "Forma_de_Nariz" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Forma_de_Nariz' + rowIndex));}DisabledControl($("#" + nameOfTable + "Grosor_de_Labios" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Grosor_de_Labios' + rowIndex));}DisabledControl($("#" + nameOfTable + "Forma_de_Menton" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Forma_de_Menton' + rowIndex));}DisabledControl($("#" + nameOfTable + "Senas_Particulares" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Senas_Particulares' + rowIndex));}DisabledControl($("#" + nameOfTable + "Imagen_Tatuaje" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Imagen_Tatuaje' + rowIndex));}DisabledControl($("#" + nameOfTable + "Otras_Senas_Particulares" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Otras_Senas_Particulares' + rowIndex));}DisabledControl($("#" + nameOfTable + "Imputado_Recluido" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Imputado_Recluido' + rowIndex));}DisabledControl($("#" + nameOfTable + "Poblacion_Hechos" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Poblacion_Hechos' + rowIndex));}DisabledControl($("#" + nameOfTable + "Poblacion_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Poblacion_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Latitud_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Latitud_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Longitud_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Longitud_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Especifique" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Especifique' + rowIndex));}DisabledControl($("#" + nameOfTable + "Modulo_Atencion_Inicial" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Modulo_Atencion_Inicial' + rowIndex));}DisabledControl($("#" + nameOfTable + "Incompleto" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Incompleto' + rowIndex));}DisabledControl($("#" + nameOfTable + "Persona_Moral" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Persona_Moral' + rowIndex));}DisabledControl($("#" + nameOfTable + "Calidad_Juridica" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Calidad_Juridica' + rowIndex));}DisabledControl($("#" + nameOfTable + "Razon_Social" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Razon_Social' + rowIndex));}DisabledControl($("#" + nameOfTable + "Nombres2" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Nombres2' + rowIndex));}DisabledControl($("#" + nameOfTable + "Apellido_Paterno2" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Apellido_Paterno2' + rowIndex));}DisabledControl($("#" + nameOfTable + "Apellido_Materno2" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Apellido_Materno2' + rowIndex));}DisabledControl($("#" + nameOfTable + "Nombre_Completo2" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Nombre_Completo2' + rowIndex));}DisabledControl($("#" + nameOfTable + "Sexo2" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Sexo2' + rowIndex));}DisabledControl($("#" + nameOfTable + "Nacionalidad2" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Nacionalidad2' + rowIndex));}DisabledControl($("#" + nameOfTable + "Escolaridad2" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Escolaridad2' + rowIndex));}DisabledControl($("#" + nameOfTable + "ID_Estado_Institucion" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'ID_Estado_Institucion' + rowIndex));}DisabledControl($("#" + nameOfTable + "Expediente_MP" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Expediente_MP' + rowIndex));}DisabledControl($("#" + nameOfTable + "Coordenada_X" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Coordenada_X' + rowIndex));}DisabledControl($("#" + nameOfTable + "Coordenada_Y" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Coordenada_Y' + rowIndex));}DisabledControl($("#" + nameOfTable + "Gravidez" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Gravidez' + rowIndex));}DisabledControl($("#" + nameOfTable + "Expediente_MASC" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Expediente_MASC' + rowIndex));}DisabledControl($("#" + nameOfTable + "Se_Informaron_sus_Derechos" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Se_Informaron_sus_Derechos' + rowIndex));}DisabledControl($("#" + nameOfTable + "Se_Informo_el_Procedimiento" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Se_Informo_el_Procedimiento' + rowIndex));}DisabledControl($("#" + nameOfTable + "Situacion_del_Imputado" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Situacion_del_Imputado' + rowIndex));}DisabledControl($("#" + nameOfTable + "Municipio_Detencion" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Municipio_Detencion' + rowIndex));}DisabledControl($("#" + nameOfTable + "Coorporacion" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Coorporacion' + rowIndex));}DisabledControl($("#" + nameOfTable + "Suspencion_Condicional" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Suspencion_Condicional' + rowIndex));}DisabledControl($("#" + nameOfTable + "Fecha_Suspencion_Condicional" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Fecha_Suspencion_Condicional' + rowIndex));} if('true' == 'true')
{
	$('#divLugar_de_Detencion a').attr('disabled', 'disabled');
	$('#divLugar_de_Detencion a').css("pointer-events","none");
}
else
{
	$('#divLugar_de_Detencion a').attr('disabled', '');
	$('#divLugar_de_Detencion a').css("pointer-events","all");
} if('true' == 'true')
{
	$('#divAdicciones_Probable_Responsable a').attr('disabled', 'disabled');
	$('#divAdicciones_Probable_Responsable a').css("pointer-events","none");
}
else
{
	$('#divAdicciones_Probable_Responsable a').attr('disabled', '');
	$('#divAdicciones_Probable_Responsable a').css("pointer-events","all");
} if('true' == 'true')
{
...
{
	$('#divOtros_Nombres a').attr('disabled', 'disabled');
	$('#divOtros_Nombres a').css("pointer-events","none");
}
else
{
	$('#divOtros_Nombres a').attr('disabled', '');
	$('#divOtros_Nombres a').css("pointer-events","all");
} if('true' == 'true')
{
...
//BusinessRuleId:4747, Attribute:0, Operation:Object, Event:SCREENOPENING
if(operation == 'New'){
 $('#divFecha_de_Detencion').css('display', 'none'); SetNotRequiredToControl( $('#' + nameOfTable + 'Fecha_de_Detencion' + rowIndex));$('#divMunicipio_Detencion').css('display', 'none'); SetNotRequiredToControl( $('#' + nameOfTable + 'Municipio_Detencion' + rowIndex));$('#divCoorporacion').css('display', 'none'); SetNotRequiredToControl( $('#' + nameOfTable + 'Coorporacion' + rowIndex));$('#divSuspencion_Condicional').css('display', 'none'); SetNotRequiredToControl( $('#' + nameOfTable + 'Suspencion_Condicional' + rowIndex));$('#divFecha_Suspencion_Condicional').css('display', 'none'); SetNotRequiredToControl( $('#' + nameOfTable + 'Fecha_Suspencion_Condicional' + rowIndex));


}
//BusinessRuleId:4747, Attribute:0, Operation:Object, Event:SCREENOPENING

//BusinessRuleId:4748, Attribute:0, Operation:Object, Event:SCREENOPENING
if(operation == 'Update'){
if( GetValueByControlType($('#' + nameOfTable + 'Situacion_del_Imputado' + rowIndex),nameOfTable,rowIndex)==TryParseInt('1', '1') ) { $('#divFecha_de_Detencion').css('display', 'block');$('#divMunicipio_Detencion').css('display', 'block');$('#divCoorporacion').css('display', 'block');$('#divSuspencion_Condicional').css('display', 'block');$('#divFecha_Suspencion_Condicional').css('display', 'block');} else { $('#divFecha_de_Detencion').css('display', 'none'); SetNotRequiredToControl( $('#' + nameOfTable + 'Fecha_de_Detencion' + rowIndex));$('#divMunicipio_Detencion').css('display', 'none'); SetNotRequiredToControl( $('#' + nameOfTable + 'Municipio_Detencion' + rowIndex));$('#divCoorporacion').css('display', 'none'); SetNotRequiredToControl( $('#' + nameOfTable + 'Coorporacion' + rowIndex));$('#divSuspencion_Condicional').css('display', 'none'); SetNotRequiredToControl( $('#' + nameOfTable + 'Suspencion_Condicional' + rowIndex));$('#divFecha_Suspencion_Condicional').css('display', 'none'); SetNotRequiredToControl( $('#' + nameOfTable + 'Fecha_Suspencion_Condicional' + rowIndex));}


}
//BusinessRuleId:4748, Attribute:0, Operation:Object, Event:SCREENOPENING

//BusinessRuleId:4751, Attribute:0, Operation:Object, Event:SCREENOPENING
if(operation == 'New'){
 SetNotRequiredToControl( $('#' + nameOfTable + 'Situacion_del_Imputado' + rowIndex));SetNotRequiredToControl( $('#' + nameOfTable + 'Fecha_de_Detencion' + rowIndex));SetNotRequiredToControl( $('#' + nameOfTable + 'Municipio_Detencion' + rowIndex));SetNotRequiredToControl( $('#' + nameOfTable + 'Coorporacion' + rowIndex));SetNotRequiredToControl( $('#' + nameOfTable + 'Suspencion_Condicional' + rowIndex));SetNotRequiredToControl( $('#' + nameOfTable + 'Fecha_Suspencion_Condicional' + rowIndex));
...
if(operation == 'New'){
if( GetValueByControlType($('#' + nameOfTable + 'Quien_Resulte_Responsable' + rowIndex),nameOfTable,rowIndex)==TryParseInt('true', 'true') ) { $("a[href='#tabDatos_de_Media_Filiacion']").css('display', 'none'); $('#divPais_de_Nacimiento').css('display', 'none'); SetNotRequiredToControl( $('#' + nameOfTable + 'Pais_de_Nacimiento' + rowIndex));$('#divGravidez').css('display', 'none'); SetNotRequiredToControl( $('#' + nameOfTable + 'Gravidez' + rowIndex));$('#divSe_Informaron_sus_Derechos').css('display', 'none'); SetNotRequiredToControl( $('#' + nameOfTable + 'Se_Informaron_sus_Derechos' + rowIndex));$('#divSe_Informo_el_Procedimiento').css('display', 'none'); SetNotRequiredToControl( $('#' + nameOfTable + 'Se_Informo_el_Procedimiento' + rowIndex));$('#divSituacion_del_Imputado').css('display', 'none'); SetNotRequiredToControl( $('#' + nameOfTable + 'Situacion_del_Imputado' + rowIndex));$('#divFecha_de_Detencion').css('display', 'none'); SetNotRequiredToControl( $('#' + nameOfTable + 'Fecha_de_Detencion' + rowIndex));$('#divMunicipio_Detencion').css('display', 'none'); SetNotRequiredToControl( $('#' + nameOfTable + 'Municipio_Detencion' + rowIndex));$('#divCoorporacion').css('display', 'none'); SetNotRequiredToControl( $('#' + nameOfTable + 'Coorporacion' + rowIndex));$('#divSuspencion_Condicional').css('display', 'none'); SetNotRequiredToControl( $('#' + nameOfTable + 'Suspencion_Condicional' + rowIndex));$('#divFecha_Suspencion_Condicional').css('display', 'none'); SetNotRequiredToControl( $('#' + nameOfTable + 'Fecha_Suspencion_Condicional' + rowIndex)); $('#divEstado').css('display', 'none'); SetNotRequiredToControl( $('#' + nameOfTable + 'Estado' + rowIndex));$('#divMunicipio').css('display', 'none'); SetNotRequiredToControl( $('#' + nameOfTable + 'Municipio' + rowIndex));$('#divColonia').css('display', 'none'); SetNotRequiredToControl( $('#' + nameOfTable + 'Colonia' + rowIndex));$('#divPoblacion').css('display', 'none'); SetNotRequiredToControl( $('#' + nameOfTable + 'Poblacion' + rowIndex));$('#divPoblacion').css('display', 'none'); SetNotRequiredToControl( $('#' + nameOfTable + 'Poblacion' + rowIndex)); $('#divOcciso').css('display', 'none'); SetNotRequiredToControl( $('#' + nameOfTable + 'Occiso' + rowIndex));} else { $("a[href='#tabDatos_de_Media_Filiacion']").css('display', 'block'); $('#divPais_de_Nacimiento').css('display', 'block');$('#divGravidez').css('display', 'block');$('#divSe_Informaron_sus_Derechos').css('display', 'block');$('#divSe_Informo_el_Procedimiento').css('display', 'block'); $('#divEstado').css('display', 'block');$('#divMunicipio').css('display', 'block');$('#divColonia').css('display', 'block');$('#divPoblacion').css('display', 'block');$('#divPoblacion').css('display', 'block'); $('#divOcciso').css('display', 'block');}
...
//BusinessRuleId:4764, Attribute:0, Operation:Object, Event:SCREENOPENING
if(operation == 'New'){
 if('true' == 'true')
{
	$('#divLugar_de_Detencion').css('display', 'none');
...
//BusinessRuleId:4765, Attribute:0, Operation:Object, Event:SCREENOPENING
if(operation == 'Update'){
if( GetValueByControlType($('#' + nameOfTable + 'Se_Presenta_con_Detenido' + rowIndex),nameOfTable,rowIndex)==TryParseInt('true', 'true') ) { if('false' == 'true')
{
	$('#divLugar_de_Detencion').css('display', 'none');
...
if(operation == 'Update'){
if( EvaluaQuery("SELECT canalizado FROM Detalle_de_Imputado WHERE Clave = FLDD[lblClave]",rowIndex, nameOfTable)==TryParseInt('1', '1') && TryParseInt(ReplaceGLOBAL('GLOBAL[idTablero]'), ReplaceGLOBAL('GLOBAL[idTablero]'))==TryParseInt('2', '2') ) { DisabledControl($("#" + nameOfTable + "Clave" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Clave' + rowIndex));}DisabledControl($("#" + nameOfTable + "Nombre" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Nombre' + rowIndex));}DisabledControl($("#" + nameOfTable + "Apellido_Paterno" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Apellido_Paterno' + rowIndex));}DisabledControl($("#" + nameOfTable + "Apellido_Materno" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Apellido_Materno' + rowIndex));}DisabledControl($("#" + nameOfTable + "Fecha_de_Nacimiento" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Fecha_de_Nacimiento' + rowIndex));}DisabledControl($("#" + nameOfTable + "Edad" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Edad' + rowIndex));}DisabledControl($("#" + nameOfTable + "Sexo" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Sexo' + rowIndex));}DisabledControl($("#" + nameOfTable + "Estado_Civil" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Estado_Civil' + rowIndex));}DisabledControl($("#" + nameOfTable + "Tipo_de_Identificacion" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Tipo_de_Identificacion' + rowIndex));}DisabledControl($("#" + nameOfTable + "Numero_de_Identificacion" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Numero_de_Identificacion' + rowIndex));}DisabledControl($("#" + nameOfTable + "Nacionalidad" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Nacionalidad' + rowIndex));}DisabledControl($("#" + nameOfTable + "Escolaridad" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Escolaridad' + rowIndex));}DisabledControl($("#" + nameOfTable + "Ocupacion" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Ocupacion' + rowIndex));}DisabledControl($("#" + nameOfTable + "Municipio" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Municipio' + rowIndex));}DisabledControl($("#" + nameOfTable + "Codigo_Postal" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Codigo_Postal' + rowIndex));}DisabledControl($("#" + nameOfTable + "Colonia" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Colonia' + rowIndex));}DisabledControl($("#" + nameOfTable + "Calle_del_Imputado" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Calle_del_Imputado' + rowIndex));}DisabledControl($("#" + nameOfTable + "Numero_Exterior" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Numero_Exterior' + rowIndex));}DisabledControl($("#" + nameOfTable + "Numero_Interior" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Numero_Interior' + rowIndex));}DisabledControl($("#" + nameOfTable + "Telefono" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Telefono' + rowIndex));}DisabledControl($("#" + nameOfTable + "Extension" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Extension' + rowIndex));}DisabledControl($("#" + nameOfTable + "Celular" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Celular' + rowIndex));}DisabledControl($("#" + nameOfTable + "Correo_Electronico" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Correo_Electronico' + rowIndex));}DisabledControl($("#" + nameOfTable + "Pais" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Pais' + rowIndex));}DisabledControl($("#" + nameOfTable + "Forma_Cara" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Forma_Cara' + rowIndex));}DisabledControl($("#" + nameOfTable + "Cejas" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Cejas' + rowIndex));}DisabledControl($("#" + nameOfTable + "Cantidad_Cabello" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Cantidad_Cabello' + rowIndex));}DisabledControl($("#" + nameOfTable + "Implantacion_Cabello" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Implantacion_Cabello' + rowIndex));}DisabledControl($("#" + nameOfTable + "Complexion" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Complexion' + rowIndex));}DisabledControl($("#" + nameOfTable + "Color_Piel" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Color_Piel' + rowIndex));}DisabledControl($("#" + nameOfTable + "Frente" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Frente' + rowIndex));}DisabledControl($("#" + nameOfTable + "Forma_Cabello" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Forma_Cabello' + rowIndex));}DisabledControl($("#" + nameOfTable + "Color_Cabello" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Color_Cabello' + rowIndex));}DisabledControl($("#" + nameOfTable + "Calvicie" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Calvicie' + rowIndex));}DisabledControl($("#" + nameOfTable + "Color_Ojos" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Color_Ojos' + rowIndex));}DisabledControl($("#" + nameOfTable + "Forma_Ojos" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Forma_Ojos' + rowIndex));}DisabledControl($("#" + nameOfTable + "Nariz_Base" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Nariz_Base' + rowIndex));}DisabledControl($("#" + nameOfTable + "Labios" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Labios' + rowIndex));}DisabledControl($("#" + nameOfTable + "Boca" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Boca' + rowIndex));}DisabledControl($("#" + nameOfTable + "Menton" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Menton' + rowIndex));}DisabledControl($("#" + nameOfTable + "Barba" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Barba' + rowIndex));}DisabledControl($("#" + nameOfTable + "Forma_Orejas" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Forma_Orejas' + rowIndex));}DisabledControl($("#" + nameOfTable + "Tamano_Orejas" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Tamano_Orejas' + rowIndex));}DisabledControl($("#" + nameOfTable + "Tipo_Lobulo" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Tipo_Lobulo' + rowIndex));}DisabledControl($("#" + nameOfTable + "Bigote" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Bigote' + rowIndex));}DisabledControl($("#" + nameOfTable + "Situacion_Fisica" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Situacion_Fisica' + rowIndex));}DisabledControl($("#" + nameOfTable + "Alias" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Alias' + rowIndex));}DisabledControl($("#" + nameOfTable + "Quien_Resulte_Responsable" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Quien_Resulte_Responsable' + rowIndex));}DisabledControl($("#" + nameOfTable + "Pais_de_Nacimiento" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Pais_de_Nacimiento' + rowIndex));}DisabledControl($("#" + nameOfTable + "Originario_de" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Originario_de' + rowIndex));}DisabledControl($("#" + nameOfTable + "Referencia_de_Domicilio" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Referencia_de_Domicilio' + rowIndex));}DisabledControl($("#" + nameOfTable + "Narrativa_Breve_de_los_Hechos" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Narrativa_Breve_de_los_Hechos' + rowIndex));}DisabledControl($("#" + nameOfTable + "Prioridad_del_Hecho" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Prioridad_del_Hecho' + rowIndex));}DisabledControl($("#" + nameOfTable + "Fecha_del_Hecho" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Fecha_del_Hecho' + rowIndex));}DisabledControl($("#" + nameOfTable + "Hora_Aproximada_del_Hecho" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Hora_Aproximada_del_Hecho' + rowIndex));}DisabledControl($("#" + nameOfTable + "Pais_del_hecho" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Pais_del_hecho' + rowIndex));}DisabledControl($("#" + nameOfTable + "Estado_del_Hecho" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Estado_del_Hecho' + rowIndex));}DisabledControl($("#" + nameOfTable + "Municipio_del_Hecho" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Municipio_del_Hecho' + rowIndex));}DisabledControl($("#" + nameOfTable + "Colonia_del_hecho" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Colonia_del_hecho' + rowIndex));}DisabledControl($("#" + nameOfTable + "Codigo_Postal_del_hecho" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Codigo_Postal_del_hecho' + rowIndex));}DisabledControl($("#" + nameOfTable + "Calle_del_hecho" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Calle_del_hecho' + rowIndex));}DisabledControl($("#" + nameOfTable + "Entre_Calle" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Entre_Calle' + rowIndex));}DisabledControl($("#" + nameOfTable + "Y_Calle" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Y_Calle' + rowIndex));}DisabledControl($("#" + nameOfTable + "Numero_Exterior_de_los_Hechos" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Numero_Exterior_de_los_Hechos' + rowIndex));}DisabledControl($("#" + nameOfTable + "Numero_interior_de_los_hecho" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Numero_interior_de_los_hecho' + rowIndex));}DisabledControl($("#" + nameOfTable + "Latitud_de_los_hechos" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Latitud_de_los_hechos' + rowIndex));}DisabledControl($("#" + nameOfTable + "Longitud_de_los_Hechos" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Longitud_de_los_Hechos' + rowIndex));}DisabledControl($("#" + nameOfTable + "Tipo_del_lugar_del_he" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Tipo_del_lugar_del_he' + rowIndex));}DisabledControl($("#" + nameOfTable + "Peso" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Peso' + rowIndex));}DisabledControl($("#" + nameOfTable + "Estatura" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Estatura' + rowIndex));}DisabledControl($("#" + nameOfTable + "Inimputable" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Inimputable' + rowIndex));}DisabledControl($("#" + nameOfTable + "Tipo_de_Inimputabilidad" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Tipo_de_Inimputabilidad' + rowIndex));}DisabledControl($("#" + nameOfTable + "Nombre_del_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Nombre_del_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Apellido_Paterno_del_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Apellido_Paterno_del_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Apellido_Materno_del_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Apellido_Materno_del_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Nombre_Completo_del_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Nombre_Completo_del_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Fecha_de_Nacimiento_del_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Fecha_de_Nacimiento_del_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Edad_del_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Edad_del_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Sexo_del_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Sexo_del_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Estado_Civil_del_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Estado_Civil_del_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Tipo_de_Identificacion_del_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Tipo_de_Identificacion_del_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Numero_de_Identificacion_del_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Numero_de_Identificacion_del_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Nacionalidad_del_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Nacionalidad_del_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Escolaridad_del_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Escolaridad_del_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Ocupacion_del_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Ocupacion_del_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Pais_del_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Pais_del_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Estado_del_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Estado_del_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Municipio_del_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Municipio_del_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Colonia_del_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Colonia_del_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Codigo_Postal_del_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Codigo_Postal_del_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Calle_del_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Calle_del_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Numero_Exterior_del_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Numero_Exterior_del_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Numero_Interior_del_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Numero_Interior_del_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Telefono_del_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Telefono_del_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Extension_del_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Extension_del_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Celular_del_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Celular_del_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Correo_Electronico_del_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Correo_Electronico_del_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Pais_de_Origen_tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Pais_de_Origen_tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Originario_de_tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Originario_de_tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "CURP" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'CURP' + rowIndex));}DisabledControl($("#" + nameOfTable + "RFC" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'RFC' + rowIndex));}DisabledControl($("#" + nameOfTable + "CURP_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'CURP_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "RFC_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'RFC_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Probable_Responsable" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Probable_Responsable' + rowIndex));}DisabledControl($("#" + nameOfTable + "Poblacion" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Poblacion' + rowIndex));}DisabledControl($("#" + nameOfTable + "Fecha_de_Detencion" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Fecha_de_Detencion' + rowIndex));}DisabledControl($("#" + nameOfTable + "Municipio_de_Detencion" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Municipio_de_Detencion' + rowIndex));}DisabledControl($("#" + nameOfTable + "Corporacion" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Corporacion' + rowIndex));}DisabledControl($("#" + nameOfTable + "Probable_Resposable" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Probable_Resposable' + rowIndex));}DisabledControl($("#" + nameOfTable + "Tipo_de_Lugar" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Tipo_de_Lugar' + rowIndex));}DisabledControl($("#" + nameOfTable + "Descripcion" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Descripcion' + rowIndex));}DisabledControl($("#" + nameOfTable + "Numero_Telefonico" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Numero_Telefonico' + rowIndex));}DisabledControl($("#" + nameOfTable + "Redes_Sociales" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Redes_Sociales' + rowIndex));}DisabledControl($("#" + nameOfTable + "Observaciones" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Observaciones' + rowIndex));}DisabledControl($("#" + nameOfTable + "Tipo_de_identificacion" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Tipo_de_identificacion' + rowIndex));}DisabledControl($("#" + nameOfTable + "Folio_Adicciones" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Folio_Adicciones' + rowIndex));}DisabledControl($("#" + nameOfTable + "Folio_otros_Nombres" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Folio_otros_Nombres' + rowIndex));}DisabledControl($("#" + nameOfTable + "Suspension_Condicional" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Suspension_Condicional' + rowIndex));}DisabledControl($("#" + nameOfTable + "Fecha_de_Suspension_Condicional" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Fecha_de_Suspension_Condicional' + rowIndex));}DisabledControl($("#" + nameOfTable + "Se_Presenta_con_Detenido" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Se_Presenta_con_Detenido' + rowIndex));}DisabledControl($("#" + nameOfTable + "Folio_Registro_Nacional_de_Detenciones" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Folio_Registro_Nacional_de_Detenciones' + rowIndex));}DisabledControl($("#" + nameOfTable + "Nombre_Completo_Detenido" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Nombre_Completo_Detenido' + rowIndex));}DisabledControl($("#" + nameOfTable + "Apodo" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Apodo' + rowIndex));}DisabledControl($("#" + nameOfTable + "Etnia" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Etnia' + rowIndex));}DisabledControl($("#" + nameOfTable + "No_de_Hijos" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'No_de_Hijos' + rowIndex));}DisabledControl($("#" + nameOfTable + "Religion" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Religion' + rowIndex));}DisabledControl($("#" + nameOfTable + "Servicio_Medico" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Servicio_Medico' + rowIndex));}DisabledControl($("#" + nameOfTable + "Escolaridad_Detenido" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Escolaridad_Detenido' + rowIndex));}DisabledControl($("#" + nameOfTable + "Especialidad" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Especialidad' + rowIndex));}DisabledControl($("#" + nameOfTable + "Estudios_Superiores" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Estudios_Superiores' + rowIndex));}DisabledControl($("#" + nameOfTable + "Idioma" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Idioma' + rowIndex));}DisabledControl($("#" + nameOfTable + "Calidad_Migratoria" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Calidad_Migratoria' + rowIndex));}DisabledControl($("#" + nameOfTable + "Estado_de_Nacimiento" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Estado_de_Nacimiento' + rowIndex));}DisabledControl($("#" + nameOfTable + "Municipio_de_Nacimiento" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Municipio_de_Nacimiento' + rowIndex));}DisabledControl($("#" + nameOfTable + "Dialecto" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Dialecto' + rowIndex));}DisabledControl($("#" + nameOfTable + "Viene_en_Estado_de_Ebriedad" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Viene_en_Estado_de_Ebriedad' + rowIndex));}DisabledControl($("#" + nameOfTable + "Bajo_el_Efecto_de_una_Droga" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Bajo_el_Efecto_de_una_Droga' + rowIndex));}DisabledControl($("#" + nameOfTable + "Nombre_de_Droga" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Nombre_de_Droga' + rowIndex));}DisabledControl($("#" + nameOfTable + "Discapacidad_Mental" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Discapacidad_Mental' + rowIndex));}DisabledControl($("#" + nameOfTable + "Discapacidad_Fisica" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Discapacidad_Fisica' + rowIndex));}DisabledControl($("#" + nameOfTable + "Discapacidad_Sensorial" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Discapacidad_Sensorial' + rowIndex));}DisabledControl($("#" + nameOfTable + "Discapacidad_Psicosocial" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Discapacidad_Psicosocial' + rowIndex));}DisabledControl($("#" + nameOfTable + "Padecimiento_de_Enfermedad" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Padecimiento_de_Enfermedad' + rowIndex));}DisabledControl($("#" + nameOfTable + "Tamano_de_Cejas" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Tamano_de_Cejas' + rowIndex));}DisabledControl($("#" + nameOfTable + "Largo_de_Cabello" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Largo_de_Cabello' + rowIndex));}DisabledControl($("#" + nameOfTable + "Anteojos" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Anteojos' + rowIndex));}DisabledControl($("#" + nameOfTable + "Forma_de_Nariz" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Forma_de_Nariz' + rowIndex));}DisabledControl($("#" + nameOfTable + "Grosor_de_Labios" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Grosor_de_Labios' + rowIndex));}DisabledControl($("#" + nameOfTable + "Forma_de_Menton" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Forma_de_Menton' + rowIndex));}DisabledControl($("#" + nameOfTable + "Senas_Particulares" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Senas_Particulares' + rowIndex));}DisabledControl($("#" + nameOfTable + "Imagen_Tatuaje" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Imagen_Tatuaje' + rowIndex));}DisabledControl($("#" + nameOfTable + "Otras_Senas_Particulares" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Otras_Senas_Particulares' + rowIndex));}DisabledControl($("#" + nameOfTable + "Imputado_Recluido" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Imputado_Recluido' + rowIndex));}DisabledControl($("#" + nameOfTable + "Poblacion_Hechos" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Poblacion_Hechos' + rowIndex));}DisabledControl($("#" + nameOfTable + "Poblacion_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Poblacion_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Latitud_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Latitud_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Longitud_Tutor" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Longitud_Tutor' + rowIndex));}DisabledControl($("#" + nameOfTable + "Especifique" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Especifique' + rowIndex));}DisabledControl($("#" + nameOfTable + "Modulo_Atencion_Inicial" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Modulo_Atencion_Inicial' + rowIndex));}DisabledControl($("#" + nameOfTable + "Incompleto" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Incompleto' + rowIndex));}DisabledControl($("#" + nameOfTable + "Persona_Moral" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Persona_Moral' + rowIndex));}DisabledControl($("#" + nameOfTable + "Calidad_Juridica" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Calidad_Juridica' + rowIndex));}DisabledControl($("#" + nameOfTable + "Razon_Social" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Razon_Social' + rowIndex));}DisabledControl($("#" + nameOfTable + "Nombres2" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Nombres2' + rowIndex));}DisabledControl($("#" + nameOfTable + "Apellido_Paterno2" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Apellido_Paterno2' + rowIndex));}DisabledControl($("#" + nameOfTable + "Apellido_Materno2" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Apellido_Materno2' + rowIndex));}DisabledControl($("#" + nameOfTable + "Nombre_Completo2" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Nombre_Completo2' + rowIndex));}DisabledControl($("#" + nameOfTable + "Sexo2" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Sexo2' + rowIndex));}DisabledControl($("#" + nameOfTable + "Nacionalidad2" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Nacionalidad2' + rowIndex));}DisabledControl($("#" + nameOfTable + "Escolaridad2" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Escolaridad2' + rowIndex));}DisabledControl($("#" + nameOfTable + "ID_Estado_Institucion" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'ID_Estado_Institucion' + rowIndex));}DisabledControl($("#" + nameOfTable + "Expediente_MP" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Expediente_MP' + rowIndex));}DisabledControl($("#" + nameOfTable + "Coordenada_X" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Coordenada_X' + rowIndex));}DisabledControl($("#" + nameOfTable + "Coordenada_Y" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Coordenada_Y' + rowIndex));}DisabledControl($("#" + nameOfTable + "Gravidez" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Gravidez' + rowIndex));}DisabledControl($("#" + nameOfTable + "Expediente_MASC" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Expediente_MASC' + rowIndex));}DisabledControl($("#" + nameOfTable + "Se_Informaron_sus_Derechos" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Se_Informaron_sus_Derechos' + rowIndex));}DisabledControl($("#" + nameOfTable + "Se_Informo_el_Procedimiento" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Se_Informo_el_Procedimiento' + rowIndex));}DisabledControl($("#" + nameOfTable + "Situacion_del_Imputado" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Situacion_del_Imputado' + rowIndex));}DisabledControl($("#" + nameOfTable + "Municipio_Detencion" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Municipio_Detencion' + rowIndex));}DisabledControl($("#" + nameOfTable + "Coorporacion" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Coorporacion' + rowIndex));}DisabledControl($("#" + nameOfTable + "Suspencion_Condicional" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Suspencion_Condicional' + rowIndex));}DisabledControl($("#" + nameOfTable + "Fecha_Suspencion_Condicional" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Fecha_Suspencion_Condicional' + rowIndex));} DisabledControl($("#" + nameOfTable + "Occiso" + rowIndex), ("true" == "true"));if ('true'=='true'){SetNotRequiredToControl( $('#' + nameOfTable + 'Occiso' + rowIndex));}} else {}


}
//BusinessRuleId:4112, Attribute:0, Operation:Object, Event:SCREENOPENING

//BusinessRuleId:4987, Attribute:0, Operation:Object, Event:SCREENOPENING
if(operation == 'New'){
...
//BusinessRuleId:4933, Attribute:2, Operation:Object, Event:AFTERSAVING
if(operation == 'New'){
if( TryParseInt(ReplaceGLOBAL('GLOBAL[idTablero]'), ReplaceGLOBAL('GLOBAL[idTablero]'))==TryParseInt('6', '6') ) { EvaluaQuery("update Detalle_de_Imputado set Mandamiento_Judicial = GLOBAL[SpartanOperationId] where Clave=GLOBAL[keyvalueinserted]", rowIndex, nameOfTable);} else {}
...
//BusinessRuleId:4727, Attribute:265810, Operation:Object, Event:BEFORESAVINGMR
if(operation == 'New'){
if( GetValueByControlType($('#' + nameOfTable + 'Tipo_de_Lugar' + rowIndex),nameOfTable,rowIndex)==TryParseInt('null', 'null') && GetValueByControlType($('#' + nameOfTable + 'Descripcion' + rowIndex),nameOfTable,rowIndex)==TryParseInt('null', 'null') ) { alert(DecodifyText('Para guardar un renglón debe capturar por lo menos un campo', rowIndex, nameOfTable));
result=false;} else {}
...
//BusinessRuleId:4729, Attribute:265811, Operation:Object, Event:BEFORESAVINGMR
if(operation == 'New'){
if( GetValueByControlType($('#' + nameOfTable + 'Correo_Electronico' + rowIndex),nameOfTable,rowIndex)==TryParseInt('null', 'null') && GetValueByControlType($('#' + nameOfTable + 'Numero_Telefonico' + rowIndex),nameOfTable,rowIndex)==TryParseInt('null', 'null') && GetValueByControlType($('#' + nameOfTable + 'Redes_Sociales' + rowIndex),nameOfTable,rowIndex)==TryParseInt('null', 'null') && GetValueByControlType($('#' + nameOfTable + 'Observaciones' + rowIndex),nameOfTable,rowIndex)==TryParseInt('null', 'null') ) { alert(DecodifyText('Para guardar un renglón debe capturar por lo menos un campo', rowIndex, nameOfTable));
result=false;} else {}
...
function EjecutarValidacionesAntesDeGuardarMROtras_Identificaciones_Probable_Responsable(nameOfTable, rowIndex) {
    ...
    //BusinessRuleId:4730, Attribute:265812, Operation:Object, Event:BEFORESAVINGMR
if(operation == 'New'){
...
//BusinessRuleId:4731, Attribute:265889, Operation:Object, Event:BEFORESAVINGMR
if(operation == 'New'){
if( GetValueByControlType($('#' + nameOfTable + 'Estado' + rowIndex),nameOfTable,rowIndex)==TryParseInt('undefined', 'undefined') && GetValueByControlType($('#' + nameOfTable + 'Municipio' + rowIndex),nameOfTable,rowIndex)==TryParseInt('undefined', 'undefined') && GetValueByControlType($('#' + nameOfTable + 'Poblacion' + rowIndex),nameOfTable,rowIndex)==TryParseInt('undefined', 'undefined') && GetValueByControlType($('#' + nameOfTable + 'Colonia' + rowIndex),nameOfTable,rowIndex)==TryParseInt('undefined', 'undefined') && GetValueByControlType($('#' + nameOfTable + 'Codigo_Postal' + rowIndex),nameOfTable,rowIndex)==TryParseInt('null', 'null') && GetValueByControlType($('#' + nameOfTable + 'Calle' + rowIndex),nameOfTable,rowIndex)==TryParseInt('null', 'null') && GetValueByControlType($('#' + nameOfTable + 'Entre_Calle' + rowIndex),nameOfTable,rowIndex)==TryParseInt('null', 'null') && GetValueByControlType($('#' + nameOfTable + 'Y_Calle' + rowIndex),nameOfTable,rowIndex)==TryParseInt('null', 'null') && GetValueByControlType($('#' + nameOfTable + 'Numero_Exterior' + rowIndex),nameOfTable,rowIndex)==TryParseInt('null', 'null') && GetValueByControlType($('#' + nameOfTable + 'Numero_Interior' + rowIndex),nameOfTable,rowIndex)==TryParseInt('null', 'null') && GetValueByControlType($('#' + nameOfTable + 'Coordenada_X' + rowIndex),nameOfTable,rowIndex)==TryParseInt('null', 'null') && GetValueByControlType($('#' + nameOfTable + 'Coordenada_Y' + rowIndex),nameOfTable,rowIndex)==TryParseInt('null', 'null') && GetValueByControlType($('#' + nameOfTable + 'Observaciones' + rowIndex),nameOfTable,rowIndex)==TryParseInt('null', 'null') ) { alert(DecodifyText('Para guardar un renglón debe capturar por lo menos un campo', rowIndex, nameOfTable));
result=false;} else {}
...
function EjecutarValidacionesAntesDeGuardarMROtros_Nombres(nameOfTable, rowIndex) {
    var result = true;
    //BusinessRuleId:4732, Attribute:265817, Operation:Object, Event:BEFORESAVINGMR
if(operation == 'New'){
if( GetValueByControlType($('#' + nameOfTable + 'Nombres' + rowIndex),nameOfTable,rowIndex)==TryParseInt('', '') && GetValueByControlType($('#' + nameOfTable + 'Apellido_Paterno' + rowIndex),nameOfTable,rowIndex)==TryParseInt('', '') && GetValueByControlType($('#' + nameOfTable + 'Apellido_Materno' + rowIndex),nameOfTable,rowIndex)==TryParseInt('', '') ) { alert(DecodifyText('Para guardar un renglón debe capturar por lo menos un campo', rowIndex, nameOfTable));
result=false;} else {}
...
if(operation == 'New'){
 SetNotRequiredToControl( $('#' + nameOfTable + 'Nombre' + rowIndex));SetNotRequiredToControl( $('#' + nameOfTable + 'Apellido_Paterno' + rowIndex));SetNotRequiredToControl( $('#' + nameOfTable + 'Apellido_Materno' + rowIndex)); SetNotRequiredToControl( $('#' + nameOfTable + 'Nombres' + rowIndex));
...
//BusinessRuleId:4726, Attribute:265786, Operation:Object, Event:BEFORESAVINGMR
if(operation == 'New'){
if( GetValueByControlType($('#' + nameOfTable + 'Probable_Responsable' + rowIndex),nameOfTable,rowIndex)==TryParseInt('null', 'null') && GetValueByControlType($('#' + nameOfTable + 'Fecha_de_Detencion' + rowIndex),nameOfTable,rowIndex)==TryParseInt('null', 'null') && GetValueByControlType($('#' + nameOfTable + 'Municipio_de_Detencion' + rowIndex),nameOfTable,rowIndex)==TryParseInt('undefined', 'undefined') && GetValueByControlType($('#' + nameOfTable + 'Corporacion' + rowIndex),nameOfTable,rowIndex)==TryParseInt('null', 'null') && GetValueByControlType($('#' + nameOfTable + 'Fecha_de_Suspension_Condicional' + rowIndex),nameOfTable,rowIndex)==TryParseInt('null', 'null') ) { alert(DecodifyText('Para guardar un renglón debe capturar por lo menos un campo', rowIndex, nameOfTable));
result=false;} else {}
                    ...
                    if (results[0]) {
                        ...
                        for (var i = results[0].address_components.length - 1; i >= 0; i--) {
                            ...
                            if (results[0].address_components[i].types[0] == "route") {
                                $("#Calle_del_Imputado").val((results[0].address_components[i].long_name));
                            }
