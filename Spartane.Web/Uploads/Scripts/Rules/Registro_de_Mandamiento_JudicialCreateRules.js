...
function IniciaMR() {
    var grid = 'Detalle_de_Personas_a_Incluir';
...
$(document).ready(function () {
    ...
    $("form#CreateRegistro_de_Mandamiento_Judicial").on('change', '#Carpeta_de_Investigacion', function () {
        nameOfTable = '';
        rowIndex = '';
        if (GetValueByControlType($('#' + nameOfTable + 'Carpeta_de_Investigacion' + rowIndex), nameOfTable, rowIndex) != TryParseInt('null', 'null')) {
            fillMRFromQuery('Detalle_de_Personas_a_Incluir', "select DISTINCT null as Folio, Nombre_Completo_Detenido as Persona, 0 as Seleccionar, di.Clave as PersonaId "
                + " from Detalle_de_Imputado di"
                + " inner join Detalle_de_Relaciones dr on dr.Probable_Responsable = di.Clave where di.Expediente_MP = FLD[Carpeta_de_Investigacion]");
            /*CODIGO ABRIR GRID*/
            setTimeout(function () {
                IniciaMR();
            }, 500);
    ...
    //BusinessRuleId:4984, Attribute:273719, Operation:Field, Event:None
    $("form#CreateRegistro_de_Mandamiento_Judicial").on('change', '#Fecha_de_Solicitud', function () {
        nameOfTable = '';
        rowIndex = '';
        if (EvaluaQuery("SELECT DATEDIFF(DAY,CONVERT(DATE,CONVERT(VARCHAR(10),GETDATE(),103),103), CONVERT(DATE,CONVERT(VARCHAR(10),'FLD[Fecha_de_Solicitud]',103),103))", rowIndex, nameOfTable) > TryParseInt('0', '0')) { alert(DecodifyText(' La Fecha de solicitud no puede ser mayor al dia de hoy.', rowIndex, nameOfTable)); AsignarValor($('#' + nameOfTable + 'Fecha_de_Solicitud' + rowIndex), ''); } else { }
    });
...
function EjecutarValidacionesAlComienzo() {
    ...
    //BusinessRuleId:4981, Attribute:0, Operation:Object, Event:SCREENOPENING
    if (operation == 'New') {
...
function EjecutarValidacionesAntesDeGuardar() {
    ...
    if (operation == 'New') {
        if (GetValueByControlType($('#' + nameOfTable + 'Carpeta_de_Investigacion' + rowIndex), nameOfTable, rowIndex) == TryParseInt('null', 'null') && GetValueByControlType($('#' + nameOfTable + 'Causa_Penal' + rowIndex), nameOfTable, rowIndex) == TryParseInt('null', 'null') && GetValueByControlType($('#' + nameOfTable + 'Fuente' + rowIndex), nameOfTable, rowIndex) != TryParseInt('2', '2')) {
            alert(DecodifyText('Es necesario seleccionar una capeta de investigacion o causa penal.', rowIndex, nameOfTable));
...
function EjecutarValidacionesNewRowMRDetalle_de_Personas_a_Incluir(nameOfTable, rowIndex) {
    ...
    //BusinessRuleId:4983, Attribute:273740, Operation:Object, Event:NEWROWMR
    if (operation == 'New') {
        $("#" + nameOfTable + 'Persona' + rowIndex).prop('disabled', 'disabled'); $("#" + nameOfTable + 'Persona' + rowIndex).removeClass('inputClientRequired'); $("#" + nameOfTable + 'Persona' + rowIndex).addClass('inputClientRequired-hide');
...
function EjecutarValidacionesEditRowMRDetalle_de_Personas_a_Incluir(nameOfTable, rowIndex) {
    ...
    //BusinessRuleId:4983, Attribute:273740, Operation:Object, Event:EDITROWMR
    if (operation == 'New') {
        $("#" + nameOfTable + 'Persona' + rowIndex).prop('disabled', 'disabled'); $("#" + nameOfTable + 'Persona' + rowIndex).removeClass('inputClientRequired'); $("#" + nameOfTable + 'Persona' + rowIndex).addClass('inputClientRequired-hide');
